...
int DistDump = 0;
float DistThresh = log(1000); // Points with at least this much difference from 
...
float ChangedThresh = .05;	// Or if at least this fraction of points changed class last time
char Log = 1;
char Screen = 1;
...
void TriSolve(float *M, float *x, float *Out, int D) {
	int i, j;
	float sum;
	float *pM, *pOut; // Use a pointer to speed things up. IT DOESN'T HELP
...
void KK::EStep() {
	...
	for (p=0; p<nPoints; p++) LogP[p*MaxPossibleClusters + 0] = -log(Weight[0]);
    ...
    for (cc=1; cc<nClustersAlive; cc++) {
		...
		LogRootDet = 0;
		for(i=0; i<nDims; i++) LogRootDet += log(Chol[i*nDims + i]);
...
float KK::CEM(char *CluFile = NULL, int Recurse=1)  {
	int p, c;
	int nChanged;
	int Iter;
